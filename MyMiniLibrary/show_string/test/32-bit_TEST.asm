; 	32-bit_TEST.asm
;	32-bit "show_string" routine  -- Using NASM syntax
;
;       Line #153 to #165 calling the Line #193 with Line #174's string
;
; 	Mighten Dai, July 17, 2015  21:21
;
org	0x7c00
Program_Entry:
jmp		___16_bit_entry
...
___32_bit_entry:
	...
	; Test the codes.
	;    !!! Caution !!!
	;		The offset address inside Code segment is counted from 0
	;
	mov		eax, Code_Selector
	mov		ds, eax
	mov		eax, PM_Message - ___32_bit_entry  ; PM_Message for solo result in #GP !!! -- Reboot
	mov		esi, eax
	mov		eax, Buffer_Selector
	mov		es, eax
	mov		edi, ( 3 * 80 + 4) * 2	; Line 3, Column 4.
	mov		ah, 0x09				; Color 
	call	_32_show_string
	;mov		eax, eax					; [EAX], return value: number of characters have drawn 
	
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;   Mission over, go back to RM.
	; Deal to back to 16-bit environment.
	jmp		Code16_Selector:0
	
	;;   Codes executing flow never go here, so make it the string storage
	PM_Message: db 'Here comes the 32-bit Message.'
				db 0    ; Auto-detect the NULL to terminate the string
	

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;	  Program codes under test.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; Due to the compiler's special mechanism, this method is invalid, so I have to add the 0xaa55 to the MBR manually.	
; times 510 - ( $ - $$ ) db 0xcc
; dw 0xaa55